...
- Data sanctity rule: persisted match/cache data is sacred. Never delete, replace, quarantine, reset, recreate, or bypass an existing database/cache file or remote snapshot unless the user explicitly authorizes that exact destructive action after the risk to stored data has been stated plainly. When investigating database errors, default to read-only diagnosis first and treat preserving stored data as higher priority than restoring app availability quickly.
