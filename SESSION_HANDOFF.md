...
- Added consumable-buff support to both `Recent Matches` and `Item Winrates`: cached detail rows now surface consumed `Aghanim's Scepter`, `Aghanim's Shard`, and `Moon Shard` with timing when available, plus a visible `BUFF` badge in the UI.
