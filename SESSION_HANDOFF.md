...
- Fix:
  ...
  - `pages/Database.py` now treats sync-cycle runtime errors as non-fatal for the page, so the database monitor can still render current Neon contents instead of crashing the full screen.
...
## 2026-05-20 corrupted SQLite cache startup recovery

- Follow-up on the same deployment incident: the live Streamlit error stayed `sqlite3.DatabaseError` even after the schema migration build was live, which pointed to a corrupted persisted SQLite cache file rather than a simple missing-column migration gap.
- Hardened `utils/store_factory.py` so SQLite startup now catches `sqlite3.DatabaseError`, quarantines the broken `.cache/matches.sqlite3` file, attempts snapshot restore when Google Drive persistence is configured, and otherwise recreates a fresh local SQLite cache instead of crashing the whole dashboard.
- Added regression coverage in `tests/test_store_factory.py` that writes a non-SQLite payload into `.cache/matches.sqlite3`, builds the match store, and verifies the app recovers with a working new cache plus a visible warning.
